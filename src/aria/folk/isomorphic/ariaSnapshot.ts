...
export function matchesNode(node: AriaNode | string, template: AriaTemplateNode, isDeepEqual: boolean): boolean {
  ...
  if (template.props) {
    for (const [key, value] of Object.entries(template.props)) {
      if (!matchesTextValue(node.props[key] || '', value))
        return false
    }
  }
...
export function containsList(children: (AriaNode | string)[], template: AriaTemplateNode[]): boolean {
  if (template.length > children.length)
    return false
